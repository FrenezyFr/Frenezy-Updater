...
#define ROOT                    "/"
#define APP_PATH                "/switch/Frenezy-Updater/"
#define APP_OUTPUT              "/switch/Frenezy-Updater/Frenezy-Updater.nro"
#define OLD_APP_PATH            "/switch/Frenezy-Updater.nro"
...
static bool update_sigpatches_handler(void) {
    if (downloadFile(FRENEZY_PACK_URL, TEMP_FILE, progressCallback)) {
        if (!unzip(TEMP_FILE)) {
            return false;
        }
...
static const struct Entry ENTRIES[] = {
    { "Update Hack", update_sigpatches_handler, "System restart required" },
...
static void refreshScreen(int cursor) {
    ...
    printf("\x1B[36mFrenezy-Updater: v%s\x1B[37m\n\n\n", APP_VERSION);
